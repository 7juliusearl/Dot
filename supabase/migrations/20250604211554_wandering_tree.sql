/*
  # Fix subscription sync and add logging
  
  1. Changes
    - Add logging table to track sync operations
    - Update sync function to include detailed logging
    - Add indexes for better performance
  
  2. Security
    - Maintain existing RLS policies
    - Add RLS for logging table
*/
...
-- Force a sync for all existing customers
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT customer_id FROM stripe_customers WHERE deleted_at IS NULL
  LOOP
    PERFORM sync_stripe_subscription(r);
  END LOOP;
END $$;