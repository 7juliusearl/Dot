-- 🚨 EMERGENCY FIX (FINAL): southernhoneyphotography112@gmail.com
-- User paid $27.99 yearly but webhook failed to create order record
-- FIXED: Using 'monthly' purchase_type (covers all recurring including yearly)

SELECT '🚨 EMERGENCY FIX FOR SOUTHERNHONEYPHOTOGRAPHY112@GMAIL.COM - FINAL VERSION' as emergency_fix;

-- Create the missing order record with correct purchase_type
...
SELECT 
  ...
  customer_id,
  2799,  -- amount_subtotal: $27.99 in cents
  2799,  -- amount_total: $27.99 in cents
  'usd',
  'paid',
  'completed',
  'monthly',  -- FIXED: Use 'monthly' for all recurring (yearly is monthly with 1-year period)
  email,
  'sub_fix_southernhoney_' || SUBSTRING(MD5(customer_id || NOW()::text) FROM 1 FOR 15),
  'active',
  'price_1RbnIfInTpoMSXouPdJBHz97',  -- EXACT yearly price ID
  EXTRACT(EPOCH FROM NOW())::bigint,
  EXTRACT(EPOCH FROM NOW() + INTERVAL '1 year')::bigint,  -- 1 year period
  false,
  'card',
  '****',
  NOW(),
...
-- Update customer record to monthly (yearly is treated as monthly with long period)
...
SELECT 
  ...
  EXTRACT(DAYS FROM (to_timestamp(current_period_end) - NOW())) as days_remaining
FROM stripe_orders 
WHERE email = 'southernhoneyphotography112@gmail.com'
...
SELECT '💰 Customer now has $27.99 yearly subscription (1-year period)' as summary;
SELECT '📱 Have customer refresh dashboard to see subscription' as instruction; 