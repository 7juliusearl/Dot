...
-- 4. Check stripe_subscriptions table (legacy data) - Only if it exists
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'stripe_subscriptions') THEN
    RAISE NOTICE 'STRIPE_SUBSCRIPTIONS table exists, checking data...';
    PERFORM 1; -- We'll add the actual query outside this block
  ELSE
    RAISE NOTICE 'STRIPE_SUBSCRIPTIONS table does not exist - this is normal in newer setups';
  END IF;
END $$;

-- Run this only if stripe_subscriptions exists
SELECT 
...
AND EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'stripe_subscriptions');
...
-- 7. General analysis of NULL fields across all users
SELECT 
...
-- 8. Check what tables actually exist in your schema
SELECT 
...
-- 10. Show recent activity if any tracking tables exist
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'sync_logs') THEN
    RAISE NOTICE 'sync_logs table found - you can check recent activity there';
  ELSE
    RAISE NOTICE 'No sync_logs table - this is normal. Check Stripe webhook logs for recent activity.';
  END IF;
END $$; 