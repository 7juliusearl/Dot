...
-- Fixed version for PostgreSQL (no ROW_COUNT function)
...
-- Step 3: Count users with NULL payment_intent_id before fix
SELECT 
  'BEFORE PAYMENT_INTENT_ID FIX:' as step,
  COUNT(*) as users_to_fix
FROM stripe_orders 
WHERE status = 'completed'
  AND deleted_at IS NULL
  AND payment_intent_id IS NULL;

-- Step 4: Fix NULL payment_intent_id for ALL users
...
-- Step 5: Verify payment_intent_id fix
SELECT 
  'AFTER PAYMENT_INTENT_ID FIX:' as step,
  COUNT(*) as users_still_null
FROM stripe_orders 
WHERE status = 'completed'
  AND deleted_at IS NULL
  AND payment_intent_id IS NULL;

-- Step 6: Count users with incomplete subscription data before fix
SELECT 
  'BEFORE SUBSCRIPTION DATA FIX:' as step,
  COUNT(*) as users_to_fix
FROM stripe_orders 
WHERE status = 'completed'
  AND deleted_at IS NULL
...
-- Step 7: Fix incomplete subscription data for monthly users
UPDATE stripe_orders 
SET 
...
-- Step 8: Count customers missing payment_type before fix
SELECT 
  'BEFORE CUSTOMER PAYMENT_TYPE FIX:' as step,
  COUNT(*) as customers_to_fix
...
-- Step 9: Fix stripe_customers records with missing payment_type
...
-- Step 10: Verify all fixes worked
SELECT 
...
-- Step 11: Show sample of fixed users
SELECT 
...
-- Step 12: Test dashboard functionality for specific users from your list
WITH test_users AS (
...
-- Step 13: Create sync_logs table and log this operation
